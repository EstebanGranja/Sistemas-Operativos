...
iobench(int N, int pid)
{
  ...
  for (int i = 0; i < N; i++){
    ...
    if (elapsed_ticks > 0) {
        metric = ((total_iops*1000) / elapsed_ticks); //operaciones por segundo, 
    } else {                                          //total_iops / elapsed_tick = operaciones reales por milisegundo (1 tick = 1 milisegundo)
                                                      //total_iops*1000 / elapsed_tick = mil * (operaciones reales) por milisegundo (lo que es igual a operaciones reales por segundo)
        metric = 0; // Evitar división por cero si no hay diferencia en ticks
    }
