...
found:
  ...
  p->state = USED;
  p->selected = 0;
  p->prio = HIGH; // Inicializa la prioridad a HIGH por defecto
...
fork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  np->prio = p->prio; // Copia la prioridad del padre al hijo
  pid = np->pid;
...
procdump(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    //printf("\n");
    printf("ESTADO: %s      PRIORIDAD de %s: %d         VECES SELECCIONADO: %d\n", state, p->name, p->prio, p->selected); // Imprime la prioridad de cada proceso
  }
}